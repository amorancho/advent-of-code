      ...
      for (let i = 0; i < value.points.length; i++) {
        for (let j = i + 1; j < value.points.length; j++) {
          ...
          while (true) {
            ...
            if (
              newAntinodeA[0] >= 0 &&
              newAntinodeA[0] < max_x &&
              newAntinodeA[1] >= 0 &&
              newAntinodeA[1] < max_y
            ) {
              pointStr =
                newAntinodeA[0].toString() + "," + newAntinodeA[1].toString();
          ...
          while (true) {
            ...
            if (
              newAntinodeB[0] >= 0 &&
              newAntinodeB[0] < max_x &&
              newAntinodeB[1] >= 0 &&
              newAntinodeB[1] < max_y
            ) {
              pointStr =
                newAntinodeB[0].toString() + "," + newAntinodeB[1].toString();
              if (!antinodes.includes(pointStr)) {
                antinodes.push(pointStr);
              }
            } else {
