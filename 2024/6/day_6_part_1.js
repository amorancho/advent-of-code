...
function doUp(pos) {
  if (pos.x > 0) {
    if ((map[pos.x-1][pos.y] === obstruction) || (map[pos.x-1][pos.y] === new_obstruction)) {
...
function doLeft(pos) {
  if (pos.y > 0) {
    if ((map[pos.x][pos.y-1] === obstruction) || (map[pos.x][pos.y-1] === new_obstruction)) {
...
function posDirExist(pos) {
  for (var i = 0; i < posDirArray.length; i++) {
    if ((posDirArray[i].x === pos.x) && (posDirArray[i].y === pos.y) && (posDirArray[i].d === pos.d)) {
      return true;
    }
  }
  return false;
